...
@interface ScanCodeViewController ()<CodeReaderViewDelegate,AVCaptureMetadataOutputObjectsDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate>
{
    CodeReaderView * readview;//二维码扫描对象
...
- (void)InitScan
{
    ...
    readview = [[CodeReaderView alloc]initWithFrame:CGRectMake(0, 0, DeviceMaxWidth, DeviceMaxHeight)];
...
{
    NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:str,@"result", nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"scanCode" object:nil userInfo:dic];
    if (str) {
